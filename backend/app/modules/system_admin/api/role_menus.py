...
async def set_role_menus(
    ...
    menu_ids = [require_int_id(item, "菜单节点不存在") for item in _unique_keep_order(body.menu_ids)]
    if menu_ids:
        found = {
            str(item)
            for item in (
                await session.execute(
                    select(MenuNode.id).where(MenuNode.id.in_([int(item) for item in menu_ids]))
                )
            ).scalars().all()
        }
